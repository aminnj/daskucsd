...
python3 convert_awkward.py $INPUTNAME

STATUS=$?
if [[ $STATUS != 0 ]]; then
    echo "Removing output file because we crashed with exit code $STATUS"
    rm table.awkd
    exit 1
fi
...
COPY_SRC="file://`pwd`/table.awkd"
COPY_DEST="gsiftp://gftp.t2.ucsd.edu${OUTPUTNAME}"
